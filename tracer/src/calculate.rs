...
pub fn calculate_trace(trace_input: &ListSerializable<usize>, lru_type: &str) -> (Hist, Hist, Reuse, Reuse){
    ...
    let mut dist_rd: ListSerializable<(usize, Option<usize>)> = ListSerializable::<(usize, Option<usize>)>::new();
    let mut dist_ri: ListSerializable<(usize, Option<usize>)> = ListSerializable::<(usize, Option<usize>)>::new();
