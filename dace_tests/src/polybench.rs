...
pub fn symm(n: usize, m: usize) -> Rc<Node> {
    ...
    let mut s_ref_b1: Rc<Node> = Node::new_ref("b1", vec![m,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);
    let mut s_ref_a1 = Node::new_ref("a1", vec![m,m], |ijk| vec![ijk[0] as usize, ijk[2] as usize]);
    let mut s_ref_c1 = Node::new_ref("c1", vec![m,n], |ijk| vec![ijk[2] as usize, ijk[1] as usize]);
    let mut s_ref_c2 = Node::new_ref("c2", vec![m,n], |ijk| vec![ijk[2] as usize, ijk[1] as usize]);

    // creating tempt2 += b[k][j] * a[i][k]
    let mut s_ref_b2 = Node::new_ref("b2", vec![m,n], |ijk| vec![ijk[2] as usize, ijk[1] as usize]);
    let mut s_ref_a2 = Node::new_ref("a2", vec![m,m], |ijk| vec![ijk[0] as usize, ijk[2] as usize]);

    // creating c[i][j] = beta * c[i][j] + alpha* b[i][j] * a[i][i] + alpha * temp2
    let mut s_ref_c3 = Node::new_ref("c3", vec![m,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);
    let mut s_ref_b3 = Node::new_ref("b3", vec![m,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);
    let mut s_ref_a3 = Node::new_ref("a3", vec![m,m], |ijk| vec![ijk[0] as usize, ijk[0] as usize]);
    let mut s_ref_c4 = Node::new_ref("c4", vec![m,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);
...
pub fn stencil(n: usize) -> Rc<Node> {
    ...
    let mut s_ref_a1: Rc<Node> = Node::new_ref("a1", vec![n,n], |ij| vec![ij[0] as usize, ij[1] as usize]);
...
pub fn seidel_2d(m: usize, n: usize) -> Rc<Node> {
    ...
    let mut s_ref_a1: Rc<Node> = Node::new_ref("a1", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize]);
    let mut s_ref_a2 = Node::new_ref("a2", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize]);
    let mut s_ref_a3 = Node::new_ref("a3", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize]);
    let mut s_ref_a4 = Node::new_ref("a4", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize]);
    let mut s_ref_a5 = Node::new_ref("a5", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize]);
    let mut s_ref_a6 = Node::new_ref("a6", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize]);
    let mut s_ref_a7 = Node::new_ref("a7", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize]);
    let mut s_ref_a8 = Node::new_ref("a8", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize]);
    let mut s_ref_a9 = Node::new_ref("a9", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize]);
    let mut s_ref_a0 = Node::new_ref("a0", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize]);
...
pub fn ludcmp(n: usize) -> Rc<Node> {
    ...
    // creating w = A[i][j]
    let mut s_ref_a1: Rc<Node> = Node::new_ref("a1", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);

    // creating w -= A[i][k] * A[k][j]
    let mut s_ref_a2 = Node::new_ref("a2", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[2] as usize]);
    let mut s_ref_a3 = Node::new_ref("a3", vec![n,n], |ijk| vec![ijk[2] as usize, ijk[1] as usize]);

    // creating A[i][j] = w / A[j][j]
    let mut s_ref_a4 = Node::new_ref("a4", vec![n,n], |ijk| vec![ijk[1] as usize, ijk[1] as usize]);
    let mut s_ref_a5 = Node::new_ref("a5", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);

    // creating w = A[i][j]
    let mut s_ref_a6 = Node::new_ref("a6", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);

    // creating w -= A[i][k] * A[k][j]
    let mut s_ref_a7 = Node::new_ref("a7", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[2] as usize]);
    let mut s_ref_a8 = Node::new_ref("a8", vec![n,n], |ijk| vec![ijk[2] as usize, ijk[1] as usize]);

    // creating A[i][j] = w
    let mut s_ref_a9 = Node::new_ref("a9", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);

    // creating w = b[i]
    let mut s_ref_b1 = Node::new_ref("b1", vec![n], |ijk| vec![ijk[0] as usize]);

    // creating w -= A[i][j] * y[j]
    let mut s_ref_a10 = Node::new_ref("a10", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);
    ...
    let mut s_ref_a11 = Node::new_ref("a11", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);
    let mut s_ref_x1 = Node::new_ref("x1", vec![n], |ijk| vec![ijk[1] as usize]);

    // creating x[i] = w / A[i][i]
    let mut s_ref_a12 = Node::new_ref("a12", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[0] as usize]);
...
pub fn nussinov(n: usize) -> Rc<Node> {
    ...
    let mut s_ref_if1_t1 = Node::new_ref("if1_t1", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);
    let mut s_ref_if1_t2 = Node::new_ref("if1_t2", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize - 1]);
    let mut s_ref_if1_t3 = Node::new_ref("if1_t3", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);

    // creating table[i][j] = max_score(table[i][j], table[i+1][j])
    let mut s_ref_if2_t1 = Node::new_ref("if2_t1", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);
    let mut s_ref_if2_t2 = Node::new_ref("if2_t2", vec![n,n], |ijk| vec![ijk[0] as usize + 1, ijk[1] as usize]);
    let mut s_ref_if2_t3 = Node::new_ref("if2_t3", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);

    // creating table[i][j] = max_score(table[i][j], table[i+1][j-1]+match(seq[i], seq[j]))
    let mut s_ref_if3_t1 = Node::new_ref("if3_t1", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);
    let mut s_ref_if3_t2 = Node::new_ref("if3_t2", vec![n,n], |ijk| vec![ijk[0] as usize + 1, ijk[1] as usize - 1]);
    ...
    let mut s_ref_if3_t3 = Node::new_ref("if3_t3", vec![n,n], |ijk| vec![ijk[0]as usize, ijk[1] as usize]);

    // creating table[i][j] = max_score(table[i][j], table[i+1][j-1])
    let mut s_ref_else3_t1 = Node::new_ref("else3_t1", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);
    let mut s_ref_else3_t2 = Node::new_ref("else3_t2", vec![n,n], |ijk| vec![ijk[0] as usize + 1, ijk[1] as usize - 1]);
    let mut s_ref_else3_t3 = Node::new_ref("else3_t3", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);

    // creating table[i][j] = max_score(table[i][j], table[i][k] + table[k+1][j])
    let mut s_ref_t1 = Node::new_ref("t1", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);
    let mut s_ref_t2 = Node::new_ref("t2", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[2] as usize]);
    let mut s_ref_t3 = Node::new_ref("t3", vec![n,n], |ijk| vec![ijk[2] as usize + 1, ijk[1] as usize]);
    let mut s_ref_t4 = Node::new_ref("t4", vec![n,n], |ijk| vec![ijk[0] as usize, ijk[1] as usize]);
    ...
    let mut q3 = Node::new_node(Stmt::Block(vec![s_ref_if3_t1, s_ref_if3_t2, s_ref_if3_s1, s_ref_if3_s2, s_ref_if3_t3]));
    let mut q4 = Node::new_node(Stmt::Block(vec![s_ref_else3_t1, s_ref_else3_t2, s_ref_else3_t3]));
...
pub fn jacobi_2d(m: usize, n: usize) -> Rc<Node> {
    ...
    let mut s_ref_a1 = Node::new_ref("a1", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize]);
    let mut s_ref_a2 = Node::new_ref("a2", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize - 1]);
    let mut s_ref_a3 = Node::new_ref("a3", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize + 1]);
    let mut s_ref_a4 = Node::new_ref("a4", vec![n,n], |tij| vec![tij[1] as usize + 1, tij[2] as usize]);
    let mut s_ref_a5 = Node::new_ref("a5", vec![n,n], |tij| vec![tij[1] as usize - 1, tij[2] as usize]);
    let mut s_ref_b1 = Node::new_ref("b1", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize]);

    // creating A[i][j] = 0.2 * (B[i][j] + B[i][j-1] + B[i][1+j] + B[(1+i)][j] + B[(i-1)][j]);
    let mut s_ref_b2 = Node::new_ref("b2", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize]);
    let mut s_ref_b3 = Node::new_ref("b3", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize - 1]);
    let mut s_ref_b4 = Node::new_ref("b4", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize + 1]);
    let mut s_ref_b5 = Node::new_ref("b5", vec![n,n], |tij| vec![tij[1] as usize + 1, tij[2] as usize]);
    let mut s_ref_b6 = Node::new_ref("b6", vec![n,n], |tij| vec![tij[1] as usize - 1, tij[2] as usize]);
    let mut s_ref_a6 = Node::new_ref("a6", vec![n,n], |tij| vec![tij[1] as usize, tij[2] as usize]);
