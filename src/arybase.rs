...
pub fn set_arybase(aloop: &mut Rc<LoopTNode>)  -> (HashMap::<String, usize>, usize) {
	...
	Walk::new(&aloop).filter(|node| matches!(&node.stmt, Stmt::Ref(_)))
	    .fold::<(HashMap<String,usize>, usize), _>( init,
		   |(mut tbl, mut cur_base), mut node| {
...
mod test {
	...
	let s_ref_b = LoopTNode::new_ref("B", vec![n, n], |i| {
	    vec![i[0] as usize, i[0] as usize + 1] });
